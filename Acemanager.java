...
public class Acemanager {
    ...
    public void setMembers(Member[] members) {
        this.members = new Member[members.length]; // we use the new size, it will avoid us shrinking the array in case
                                                   // of delete
        for (int i = 0; i < this.members.length; i++) {
            this.members[i] = new Member(members[i]);
            this.members[i].setIdMember(i + 1);
    ...
    /*
     * LS after deletion: - Create a copy array.
     * - Copy all created elements to the new array until the row to delete.
     * - Ignore the row to delete, but next rows has to be shifted
     * - Copy next rows
     * - Set the new array to the member array
     * - Delete the the card linked to the member
     * - update the link between the cards and their corresponding members as the
     * member array is shifted and updated
     */
    ...
    public void deleteMember(Member member, int index) {
        ...
        for (int i = 0; i < copyMembers.length; i++) {
            // once we hit the row to delete, we copy next rows
            if (i >= index) {
        ...
        // this.shrink(this.members);- // no more needed, the array got shrinked in the
        // set method.
        ...
        // update the card member id in cards, as the members id have been altered after
        // the member deletion
    ...
    /*
     * Same logic as delete member method
     */
    ...
    public FacultyInstitute findFacultyByName(String fac) {
        for (int i = 0; i < this.facs.length; i++) {
            if (fac.equals(this.facs[i].getNameFacInst()))
                return new FacultyInstitute(this.facs[i]);
        }
        return null;
    }
    ...
    public Formation findFormationByName(String field) {
        for (int i = 0; i < this.formations.length; i++) {
            if (field.equals(formations[i].getName()))
                return new Formation(formations[i]);
        }
        return null;
    }
    ...
    /*
     * Grow any given array in order to add new elements as arrays are immutable
     * objects
     */
    ...
    public boolean checkPassport(String passport) {
        for (int i = 0; i < this.members.length; i++) {
            ...
            if (passport.equals(storedPassport))
                return true;
        }
        return false;
    }
    ...
    public boolean checkCin(String cin) {
        for (int i = 0; i < this.cards.length; i++) {
            if (cin.equals(cards[i].getCardNum()))
                return true;
        }
        return false;
    }
    ...
    public boolean checkMatAMCI(int mat) {
        for (int i = 0; i < this.members.length; i++) {
            if (mat == members[i].getMatriculeAmci())
                return true;
        }
        return false;
    ...
    public City checkCity(String city) {
        for (int i = 0; i < this.cities.length; i++) {
            if (city.equals(cities[i].getName()))
                return new City(cities[i]);
        }
        return null;
    }
